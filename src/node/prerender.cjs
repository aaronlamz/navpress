...
async function prerender() {
  ...
  try {
    const { createApp } = await server.ssrLoadModule(
      path.resolve(pkgRoot, 'src/main.js')
    )
    ...
    // 检查是否已经有CSS和JS链接，如果没有才添加
    if (!html.includes('href="/navpress/assets/')) {
      const basePath = userConfig.base || '/'
      const cssTag = `<link rel="stylesheet" href="${basePath}assets/${cssFile}">`
      html = html.replace('</head>', `${cssTag}</head>`)
    }

    if (!html.includes('src="/navpress/assets/')) {
      const basePath = userConfig.base || '/'
      const scriptTag = `<script type="module" src="${basePath}assets/${jsFile}"></script>`
      html = html.replace(
